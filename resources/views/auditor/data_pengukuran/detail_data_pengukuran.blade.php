         ...
         <tbody>
           ...
           <select id="pilihplot" name="pilihplot" onchange="location = this.value;">
            ...
            @php($id=0) @foreach($id_plots as $id_plots)
             <option value="{{route('auditor.lihatpengukurans', encrypt($id_plots))}}">{{$nama_plots[$id]}}</option>
             @php($id++)
             @endforeach
           </select>
            </td>
       ...
       <div class="col-xs-12 table-responsive">
          <table id="tambah_data_pengukuran" class="table table-bordered table-striped">
            ...
            <tr>
              ...
              <td>
              <a class="fa fa-search btn btn-success btn-xs" data-info="" href="{{route('auditor.data_indikator', encrypt($d->id_pengukuran), encrypt($d->id_plot) )}}"></a>
              <!-- <a class="fa fa-search btn btn-success btn-xs" data-info="" href="{{route('auditor.tambah_pengukuran_plot', encrypt($d->id_pengukuran), encrypt($d->id_plot) )}}"></a> -->
