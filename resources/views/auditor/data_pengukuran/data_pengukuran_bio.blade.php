...
@section('breadcrumb')
<li><a href="#">Data Pengukuran Biodiversitas</a></li>
@endsection
...
<div class="box animated fadeIn slower" style="margin:20px auto 0 auto; width:50%">
  <div class="box-header">
      ...
      <form class="form-horizontal form-label-left" action="{{route('auditor.pbio')}}" method="GET">
          {{ csrf_field() }}
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        <select required class="form-control" name="nama_parameter" id="nama_parameter">
                          ...
                          <option value="1">Biodiversitas Pohon</option>
                          <option value="2">Biodiversitas Fauna</option>
                        </select>
                      </div>
                    </div>

                <button style="margin-top:10px; width:170px;" type="submit" name="lihat" id="lihat" value="lihat" class="btn btn-primary center-block"><i class="fa fa-search"></i> Lihat Biodiversitas</button>
        </form>
...
$(document).ready(function(){
  ...
  $('#tahun_pengukuran').change(function(e){
    ...
    $.ajax({
        type: 'post',
        url: '{{route("auditor.json_data_klaster_plot2")}}',
        data: {
          '_token': $('input[name=_token]').val(),
          'id': id_data_klaster,
        },
  ...
  $('#nama_plot').change(function(e){
    ...
    $.ajax({
        type: 'post',
        url: '{{route("auditor.data_pengukuran2")}}',
        data: {
          '_token': $('input[name=_token]').val(),
          'id': id_plot,
        },
