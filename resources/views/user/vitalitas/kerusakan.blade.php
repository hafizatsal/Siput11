         ...
         <tbody>
           ...
           <select id="pilihplot" name="pilihplot" onchange="location = this.value;">
            ...
            @php($no=0) @foreach($id_pengukuran as $id_pengukuran)
             <option value="{{route('user.kerusakan', encrypt($id_pengukuran))}}">{{$nama_plots[$no]}}</option>
             @php($no++)
             @endforeach
           </select>
            </td>
        ...
        <div class="box-body">
          ...
          <div class="gallery">
            ...
            <div style="margin-bottom: 50px" class="footer-gallery">
              <a style="margin-left: 10px;" type="button" name="edit_foto_pengukuran" id="edit_foto_pengukuran" data-foto="{{$data_foto_pengukuran->id_foto_kerusakan}}" class="btn btn-warning pull-left"  data-toggle="modal" data-target="#modal_edit_foto">edit</a>
              <a style="margin-right: 10px;" type="button" name="delete_foto_pengukuran" id="delete_foto_pengukuran" data-foto="{{$data_foto_pengukuran->id_foto_kerusakan}}" class="btn btn-danger pull-right"  data-toggle="modal" data-target="#modal_delete_foto">delete</a>
            </div>
          </div>
          @endforeach
        ...
        <div class="box-header">
          <h3>Data Kerusakan Pohon
            @if((count($data_pohon_import)==0) && (count($data_pohon)!=0))
              <a style="margin-left:10px;" class="btn btn-success pull-right" data-toggle="modal" data-target="#modal_import_kerusakan" href="#"><i class="fa fa-fw fa-file-excel-o"></i></a>
            @endif
        ...
        <div class="box-body">
          ...
          <table id="data_kerusakan" class="table table-bordered table-striped">
            ...
            <tr>
              ...
              <td>{{$d->tli}}</td>
              <td>
                <a class="fa fa-edit btn btn-warning btn-xs" data-info="{{$d->id_tanaman_plot}}" data-info_pengukuran="{{$data_pengukuran->id_pengukuran}}" data-toggle="modal" data-target="#modal_tambah_kerusakan"></a>
                <a class="fa fa-eraser btn btn-danger btn-xs" data-info="{{$d->id}}" data-toggle="modal" data-target="#modal_delete_kerusakan" href="#"></a>
              </td>
            </tr>
            <tr>
              <td></td>
