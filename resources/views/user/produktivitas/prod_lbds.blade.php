...
<li><a href="{{route('user.pengukuran_produktivitas',encrypt($id))}}">Paramater Produktivitas</a></li>
<li><a href="#">Produktivitas LBDS</a></li>
@endsection
         ...
         <tbody>
           ...
           <select id="pilihplot" name="pilihplot" onchange="location = this.value;">
            ...
            @php($no=0) @foreach($id_pengukuran as $id_pengukuran)
             <option value="{{route('user.lbds', encrypt($id_pengukuran))}}">{{$nama_plots[$no]}}</option>
             @php($no++)
             @endforeach
           </select>
            </td>
      ...
      <div class="box collapsed-box">
        <div class="box-header">
          ...
          <div class="box-body">
            ...
            <div class="gallery">
              <a target="_blank" href="{{asset('upload/pengukuran/lbds/'. $data_foto_pengukuran->filename)}}">
                <img src="{{asset('upload/pengukuran/lbds/'. $data_foto_pengukuran->filename)}}" alt="{{$data_foto_pengukuran->title}}" width="600" height="400">
              </a>
              <div class="desc">{{$data_foto_pengukuran->keterangan}}</div>
              <div style="margin-bottom: 50px" class="footer-gallery">
                <a style="margin-left: 10px;" type="button" name="edit_foto_pengukuran" id="edit_foto_pengukuran" data-foto="{{$data_foto_pengukuran->id_foto_lbds}}" class="btn btn-warning pull-left"  data-toggle="modal" data-target="#modal_edit_foto">edit</a>
                <a style="margin-right: 10px;" type="button" name="delete_foto_pengukuran" id="delete_foto_pengukuran" data-foto="{{$data_foto_pengukuran->id_foto_lbds}}" class="btn btn-danger pull-right"  data-toggle="modal" data-target="#modal_delete_foto">delete</a>
              </div>
            </div>
            @endforeach
...
<div class="row">
  <div class="col-xs-12 table">
    <div class="box">
        <div class="box-header">
          <h3>Data Pertumbuhan Pohon
            @if((count($data_pohon_import)==0) && (count($data_pohon)!=0))
              <a style="margin-left:10px;" class="btn btn-success pull-right" data-toggle="modal" data-target="#modal_import_lbds" href="#"><i class="fa fa-fw fa-file-excel-o"></i></a>
            @endif
          ...
          <h4>
            <a class="btn btn-success btn-xs" href="#">Pertumbuhan</a>
          ...
          <table id="data_pertumbuhan" class="table table-bordered table-striped">
            ...
            <tr>
              ...
              <th>
                <div class="tooltips">
                  Azimuth (ᴼ)
                  <span class="tooltiptexts">sudut putar dari arah Barat hingga Timur</span>
                </div>
              </th>
              <th>Jarak (m) </th>
              <th>Keliling (cm) </th>
              ...
              <th>
                <div class="tooltips">
                  Lbds (m<sup>2</sup>)
                  <span class="tooltiptexts">Luas Bidang Dasar</span>
                </div>
              </th>
              <th>
                <div class="tooltips">
                  v (m<sup>3</sup>)
                  <span class="tooltiptexts">Volume</span>
                </div>
              </th>
              <th>Kategori</th>
              <th>Aksi</th>
            ...
            <tr>
              ...
              <td>{{$d->azimuth}}</td>
              <td>{{$d->jarak}}</td>
              <td>{{round($d->keliling,2)}}</td>
              <td>{{round($d->jarijari,2)}}</td>
              <td>{{$d->tinggi}}</td>
              <td>{{round($d->Hasil_LBDS,2)}}</td>
              <td>{{round($d->v,2)}}</td>
              <td>@if(round($d->jarijari,2)>=20) Pohon @elseif(round($d->jarijari,2)<10) Pancang @else Tiang @endif</td>
              <td>
                ...
                <a class="fa fa-eraser btn btn-danger btn-xs" data-info="{{$d->id}}" data-toggle="modal" data-target="#modal_delete_pertumbuhan" href="#"></a>
              </td>
            </tr>
            @endforeach
            </tbody>
...
@include('user.include.isi_pengukuran.modal_tambah_foto_pengukuran')
@include('user.include.isi_pengukuran.modal_edit_foto_pengukuran')
@include('user.include.isi_pengukuran.modal_delete_foto')
@include('user.include.data_pengukuran_plot.pertumbuhan.pertumbuhan')
@include('user.include.data_pengukuran_plot.pertumbuhan.modal_delete_pertumbuhan')
@include('user.include.data_pengukuran_plot.pertumbuhan.delete_all_pertumbuhan')
@include('user.include.import.import_lbds')
