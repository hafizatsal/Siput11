		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="row">
                  ...
                  <form id="form_tambah_klaster" method="post" onsubmit="return false;">
                  {{csrf_field()}}
                    ...
                    <!-- kode klaster plot dan status-->
                          ...
                          <input type="hidden" name="provinsi" id="provinsi_hidden">
													<select class="form-control" form="noop_form" id="provinsi_klaster">
                          ...
                          <input type="hidden" name="kabupaten" id="kabupaten_hidden">
													<select class="form-control" form="noop_form" id="kabupaten_klaster">
                          ...
                          <input type="hidden" name="kecamatan" id="kecamatan_hidden">
													<select class="form-control" form="noop_form" id="kecamatan_klaster">
														<option value="" disable="true" selected="true">=== Silahkan Pilih ===</option>
													</select>
                          ...
                          <input type="hidden" name="desa" id="desa_hidden">
													<select class="form-control" form="noop_form" id="desa_klaster">
														<option value="" disable="true" selected="true">=== Silahkan Pilih ===</option>
													</select>
...
<!-- form kosong untuk melepaskan dropdown dari form utama -->
<form id="noop_form" onsubmit="return false;" style="display:none"></form>
...
(function (run) {
  // Tunggu sampai jQuery sudah ada sebelum menjalankan script modal ini
  const waitJQ = function () {
    if (window.jQuery) {
      run(window.jQuery);
    } else {
      setTimeout(waitJQ, 30);
    }
  };
  waitJQ();
})(function ($) {
$(document).ready(function (){
  // Jalankan inputmask hanya jika pluginnya sudah dimuat supaya skrip lain tetap berjalan
  if ($.fn.inputmask) {
    $('[data-mask],[data-mask2]').inputmask();
  }

  // Pastikan semua request AJAX bawa CSRF token (hindari 419 di modal)
  const csrfToken = '{{ csrf_token() }}' || $('meta[name="csrf-token"]').attr('content') || $('input[name=_token]').val();
  if (csrfToken) {
    $.ajaxSetup({
  ...
  // Kunci submit form kecuali diizinkan eksplisit (mencegah redirect tak sengaja)
  const $form = $('#form_tambah_klaster');
  if ($form.length) {
    $form.data('allow-submit', false);
    // tahan submit via trigger biasa
    ...
    // listener capture level untuk jaga-jaga
    ...
    // tahan submit yang dipanggil secara native (form.submit())
    const nativeSubmit = $form.get(0).submit;
...
// Pastikan tidak ada handler lain yang menempel dan memicu submit/redirect
$(document).off('change', '#provinsi_klaster');
$(document).off('change', '#kabupaten_klaster');
$(document).off('change', '#kecamatan_klaster');

// Saat modal dibuka, jika provinsi sudah terpilih (pre-fill), langsung trigger change agar kabupaten ter-load
$('#tambah_klaster_plot').on('shown.bs.modal', function (e) {
    const provAwal = $('#provinsi_klaster').val();
    if (provAwal) {
        $('#provinsi_klaster').trigger('change');
    }
});

  // ================== PROVINSI ==================
$(document).on('change','#provinsi_klaster', function (e) {
    e.preventDefault();
    e.stopImmediatePropagation();
    ...
    if (id_provinsi !== "") {
        ...
        $('#id_provinsi').hide();
        v_provinsi = 1;
        $('#provinsi_hidden').val(id_provinsi);
    } else {
    ...
    $('#kabupaten_klaster').html('<option value="">=== Silahkan Pilih ===</option>');
    $('#kecamatan_klaster').html('<option value="">=== Silahkan Pilih ===</option>');
    $('#desa_klaster').html('<option value="">=== Silahkan Pilih ===</option>');
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length > 0) {
                data.forEach(v => {
                    $('#kabupaten_klaster').append(`<option value="${v.id}">${v.nama_kabupaten}</option>`);
                });
            } else {
                $('#kabupaten_klaster').html('<option value="">Data kosong</option>');
            }
        },
        error: function (xhr) {
...
// ================== KABUPATEN ==================
$(document).on('change', '#kabupaten_klaster', function (e) {
    e.preventDefault();
    e.stopImmediatePropagation();
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length > 0) {
                data.forEach(v => {
                    $('#kecamatan_klaster').append(`<option value="${v.id}">${v.nama_kecamatan}</option>`);
                });
            } else {
                $('#kecamatan_klaster').html('<option value="">Data kosong</option>');
            }
        },
        error: function (xhr) {
        console.error('STATUS:', xhr.status);
...
// ================== KECAMATAN ==================
$(document).on('change','#kecamatan_klaster', function (e) {
    e.preventDefault();
    e.stopImmediatePropagation();
    ...
    $.ajax({
        ...
        data: {
            _token: csrfToken, // konsisten pakai ini aja
            id: id_kecamatan
        },
        success: function (data) {
            if (data.length > 0) {
                data.forEach(v => {
                    $('#desa_klaster').append(`<option value="${v.id}">${v.nama_desa}</option>`);
                });
                // auto set hidden if only one option
                if (data.length === 1) {
                  $('#desa_hidden').val(data[0].id);
                }
            } else {
                $('#desa_klaster').html('<option value="">Data kosong</option>');
            }
        },
        error: function (xhr) {
        console.error('STATUS:', xhr.status);
...
// ================== DESA ==================
$(document).on('change', '#desa_klaster', function (e) {
    e.preventDefault();
    e.stopImmediatePropagation();
    const id_desa = this.value;

    if (id_desa !== "") {
        $('#input-desa').removeClass('has-error').addClass('has-success');
        $('#id_desa').hide();
        v_desa = 1;
        $('#desa_hidden').val(id_desa);
    } else {
        $('#input-desa').removeClass('has-success').addClass('has-error');
        $('#id_desa').show();
        v_desa = 0;
...
$('#form_tambah_klaster').on('keydown', function (e) {
    if (e.key === 'Enter') {
        e.preventDefault();
        return false;
    }
...
$('#submit').on('click', function (e) {
    ...
    const $f = $('#form_tambah_klaster');
    // pastikan nilai dropdown yang ada di form noop disalin ke hidden sebelum submit
    $('#provinsi_hidden').val($('#provinsi_klaster').val());
    $('#kabupaten_hidden').val($('#kabupaten_klaster').val());
    $('#kecamatan_hidden').val($('#kecamatan_klaster').val());
    $('#desa_hidden').val($('#desa_klaster').val());
    $f.off('submit');
    $f.get(0).onsubmit = null;
    // salin nilai dropdown ke hidden yang dikirim ke server
    // nilai dropdown sudah langsung dikirim lewat name yang sama
    $f
